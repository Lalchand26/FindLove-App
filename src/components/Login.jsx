...
const Login = () => {
  ...
  // 2. PASSWORD RESET LINK FUNCTION (Hardcoded redirect URL for production)
  const sendResetEmail = async (targetEmail) => {
    const emailToSend = targetEmail?.trim() || email.trim()

    if (!emailToSend) {
      toast.error('Kripya pehle email address enter karein')
      return
    }

    setResetLoading(true)
    try {
      const redirectUrl = 'https://www.citycrossed.com/reset-password'
  ...
  const handleForgotPassword = async () => {
    ...
    // Agar user ne Step 1 par email nahi dala hai, toh prompt se le lenge
    if (!resetEmail) {
      const inputEmail = window.prompt('Kripya apna registered email address dalein:')
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md">
        ...
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c20.08-1.92 3.28-4.74 3.28-8.09z"/>
        ...
        <div className="text-center mt-4">
          <button 
            ...
            className="text-pink-600 text-sm hover:underline disabled:opacity-50 font-medium"
          >
