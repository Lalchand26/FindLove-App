...
function ProfileTab({ session, onProfileUpdate }) {
  ...
  const [profile, setProfile] = useState({ full_name: '', username: '', age: '', gender: '', country: '', avatar_url: '' }); // username, age, gender add kiya
  ...
  const getProfile = async () => {
    try {
      ...
      .from('profiles')
      .select('full_name, username, age, gender, country, avatar_url') // sab field lelo
      .eq('id', session.user.id)
      .single();

      if (error && error.code!== 'PGRST116') throw error; // new user ke liye
  ...
  const handlePhotoUpload = async (e) => {
    try {
      ...
      .from('avatars')
      .upload(filePath, file, { upsert: true }); // upsert add kiya

      if (uploadError) throw uploadError;

      const { data: { publicUrl } = supabase.storage // {} ka bracket theek kiya
      .from('avatars')
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      const { error } = await supabase.from('profiles').upsert({
        ...
        username: profile.username, // add kiya
        age: profile.age, // add kiya
        gender: profile.gender, // add kiya
  ...
  // 👇 MOBILE TEXT FIX -!important wala
  const inputClass = "w-full p-3 rounded-xl border-2 bg-white!text-black placeholder:!text-gray-500 border-gray-300 focus:border-pink-500 focus:outline-none dark:bg-gray-700 dark:!text-white dark:border-gray-600"
  ...
  return (
    <div className="max-w-md mx-auto bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-md border-gray-100 dark:border-gray-700">
      ...
      <form onSubmit={handleSaveProfile} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className={inputClass + " border-red-400"} // red border
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">⚧️ GENDER</label>
            <select
        ...
        <div>
          <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">🌍 SELECT REGIONAL COUNTRY</label>
          <input
            type="text"
        ...
        {/* Action Controls */}
        <button type="submit" disabled={loading} className="w-full bg-pink-500 hover:bg-pink-600 text-white font-semibold py-2.5 rounded-xl shadow transition disabled:opacity-50">
      ...
      {/* Delete Zone */}
      <div className="p-3 bg-red-50 dark:bg-red-950/20 rounded-xl border-red-200">
