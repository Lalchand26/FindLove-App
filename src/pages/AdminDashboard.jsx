...
export default function AdminDashboard({ session }) {
  ...
  const fetchReports = async () => {
    ...
    setReports(data.map(r => ({...r, reporter_name: r.reporter?.full_name, reporter_email: r.reporter?.email, reported_name: r.reported_user?.full_name, reported_email: r.reported_user?.email })))
  }
  ...
  const handleDeleteUser = async (userId, name) => {
    if (!window.confirm(`${name} ko delete kare?`)) return
    await supabase.from('profiles').delete().eq('id', userId)
    toast.success(`${name} Deleted`)
    loadAllData()
  }

  if (loading) return <div className="flex h-screen items-center justify-center"><Shield className="w-12 h-12 animate-pulse text-pink-500" /></div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => {supabase.auth.signOut(); navigate('/login')}} className="flex items-center gap-2 bg-red-500 text-white px-4 py-2 rounded-lg"><LogOut />Logout</button>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white dark:bg-gray-800 p-4 rounded-xl"><p className="text-xs text-gray-500">TOTAL USERS</p><p className="text-2xl font-bold text-blue-600">{stats.totalUsers}</p></div>
          <div className="bg-white dark:bg-gray-800 p-4 rounded-xl"><p className="text-xs text-gray-500">PENDING REPORTS</p><p className="text-2xl font-bold text-yellow-600">{stats.pendingReports}</p></div>
          <div className="bg-white dark:bg-gray-800 p-4 rounded-xl"><p className="text-xs text-gray-500">TOTAL BLOCKS</p><p className="text-2xl font-bold text-red-600">{stats.totalBlocks}</p></div>
          <div className="bg-white dark:bg-gray-800 p-4 rounded-xl"><p className="text-xs text-gray-500">TOTAL REPORTS</p><p className="text-2xl font-bold text-purple-600">{stats.totalReports}</p></div>
        </div>
        ...
        <div className="flex gap-2 mb-6 bg-white dark:bg-gray-800 p-2 rounded-xl">
          ...
          <button onClick={() => setActiveView('users')} className={`flex-1 p-2 rounded-lg font-bold ${activeView === 'users'? 'bg-pink-500 text-white' : ''}`}><Users className="w-4 h-4 inline mr-2" />Users ({allUsers.length})</button>
        </div>

        {activeView === 'reports' && (
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6">
            <h2 className="text-xl font-semibold mb-4"><Eye /> Reports - Kisne Kisko Report Kiya</h2>
            {reports.map((report) => (
              <div key={report.id} className="border p-4 rounded-lg mb-4">
                <div className="flex justify-between">
                  <div>
                    <p className="font-bold">{report.reporter_name} <span className="text-xs">({report.reporter_email})</span></p>
                ...
                <p className="text-xs mt-1"><Clock className="w-3 h-3 inline" /> {new Date(report.created_at).toLocaleString('en-IN')}</p>
                {report.status === 'pending' && (
                  <div className="flex gap-2 mt-3">
                    <button onClick={() => handleReportAction(report.id, 'approve', report.reported_user_id, report.reported_name)} className="bg-red-500 text-white px-3 py-2 rounded-lg text-sm"><UserX /> Ban User</button>
                    <button onClick={() => handleReportAction(report.id, 'dismiss', report.reported_user_id, report.reported_name)} className="bg-gray-500 text-white px-3 py-2 rounded-lg text-sm">Dismiss</button>
                  </div>
                )}
              </div>
            ))}
